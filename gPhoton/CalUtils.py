...
def xieta2colrow(xi, eta, detsize, fill, npixx, npixy):
    ...
    col = ((xi / 36000.0) / (detsize / 2.0) * fill + 1.0) / 2.0 * npixx

    row = ((eta / 36000.0) / (detsize / 2.0) * fill + 1.0) / 2.0 * npixy
...
def stimcount(data, band, t0, t1, margin=90.001):
    ...
    eclipse = 100.0 if isPostCSP(t0) else 40000.0  # HACK: for backwards comp.
...
# ------------------------------------------------------------------------------
def deadtime_method1(
    data, t0, t1, band, feeclkratio=0.966, tec2fdead=5.52e-6, tstep=1.0
):
    """
    Given a dict() containing 't', a list of global event times, computes
    ...
    bins = (
        np.linspace(0.0, exptime - exptime % tstep, exptime // tstep + 1) + t0
    )
...
def deadtime_method2(
    ...
    return (1.0 - ((h[ix] / tstep) / feeclkratio) / refrate).mean()


# ------------------------------------------------------------------------------

# ------------------------------------------------------------------------------
def deadtime_fromlist(
    photonfile,
    t0,
    t1,
    band,
    ...
    return {0: deadtime_method0, 1: deadtime_method1, 2: deadtime_method2}[
        method
    ](data, t0, t1, band)
...
def raw6_to_stims(raw6file, band, eclipse, margin=90.001):
    ...
    for i in range(int(nphots / chunksz) + 1):
        ...
        if chunkend > nphots:
            chunkend = nphots - 1
        chunkid = (
            " " + str(i + 1) + " of " + str(int(nphots / chunksz) + 1) + ": "
        )
        ...
        index1, index2, index3, index4 = find_stims_index(
            x, y, band, eclipse, margin
        )

        append_events_to_stims(
            index1,
            index2,
            index3,
            index4,
...
def append_events_to_stims(
    ...
    stim1["yap"] = np.append(
        stim1["yap"], rtaph_yap(ya[index1], yb[index1], yamc[index1])
    )
    ...
    stim2["yap"] = np.append(
        stim2["yap"], rtaph_yap(ya[index2], yb[index2], yamc[index2])
    )
    ...
    stim3["yap"] = np.append(
        stim3["yap"], rtaph_yap(ya[index3], yb[index3], yamc[index3])
    )
    ...
    stim4["yap"] = np.append(
        stim4["yap"], rtaph_yap(ya[index4], yb[index4], yamc[index4])
    )
